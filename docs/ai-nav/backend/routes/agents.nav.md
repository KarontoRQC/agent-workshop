...
定义数据库智能体目录的 HTTP 边界。前端通过这里读取完整智能体列表、真实 GPT 启动链接、知识库字段和头像 URL；`avatar_url` 优先返回 nginx/静态目录图片地址，旧头像接口保留为兜底。
...
| `get_agent_avatar_url` | function | ~51 | 优先从 store 获取静态头像 URL，失败时回退旧头像接口。 |
...
- **改头像返回方式**: 保持 `avatar_url` 是可被 `<img>` 直接加载的静态 URL，不要把图片二进制塞进 `GET /api/agents`。
