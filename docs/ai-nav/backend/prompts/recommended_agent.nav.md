...
指导推荐智能体从可用智能体集合中选择 1-10 个，并按固定 XML 结构输出推荐列表和组合总结。默认候选清单不包含源表里缺少启动链接的智能体。
...
- **修改候选名称规则**: 同步 `data/source_agents_full.json` 字段读取逻辑；没有 `智能体链接` 的智能体不能写入 prompt 默认候选清单。
