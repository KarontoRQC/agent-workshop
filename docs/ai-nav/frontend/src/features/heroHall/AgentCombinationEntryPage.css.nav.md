...
定义组合入口页的蓝金 cockpit HUD 视觉：整页殿堂背景、顶部右上保存按钮、顶部殿堂舞台、首席智能体展柜、精选场景、推荐/目录卡片网格、组合智能体阵容、评分表、可选智能体类目筛选、拖拽浮影、等级徽章、状态面板和响应式布局。
...
| `.agent-combination-save-bar` | 组合页右上角保存按钮和保存反馈的 sticky 容器。 |
| `.agent-combination-save-button` | 保存当前五槽组合阵容的按钮、保存中/已保存/禁用态。 |
...
- **改保存按钮视觉**: 调整 `.agent-combination-save-bar`、`.agent-combination-save-button` 和 `.agent-combination-save-feedback`，保持右上角位置和移动端不遮挡 Hero。
- **改组合阵容视觉**: 同步检查 `.agent-combination-lineup-builder`、`.agent-combination-lineup-slot`、`.agent-combination-lineup-open`、`.agent-combination-score-panel`、`.agent-combination-candidate-categories` 和 `.agent-combination-candidate-card`；组合智能体头部不保留独立评分角标。
