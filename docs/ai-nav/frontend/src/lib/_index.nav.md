...
| `recommendationSnapshotClient.ts` | 读取 `GET /api/recommendations/<id>` 推荐快照，并通过 `PUT /api/recommendations/<id>/lineup` 保存当前组合阵容。 | `fetchRecommendationSnapshot`, `saveRecommendationLineup`, `RecommendationSnapshotError` |
