...
The committed `.env` is intentional for this private prototype. It keeps Vite proxying `/api` to the shared Agent Workshop API and keeps speech output in `server` mode, so the app requests `/api/tts/speech` and does not auto-fallback to browser `speechSynthesis`.
