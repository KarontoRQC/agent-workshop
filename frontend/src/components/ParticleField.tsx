...
const IDLE_FRAME_RATE = 48;
const BACKGROUND_FRAME_RATE = 28;
const HIDDEN_FRAME_RATE = 8;
...
export default function ParticleField({ audioLevel, graphFocusKey = '', graphRoute = [], performanceMode = 'active', settings }: ParticleFieldProps) {
  ...
  useEffect(() => {
    ...
    const getRenderPixelRatio = (nextWidth = host.clientWidth || window.innerWidth) => {
      const pixelRatio = window.devicePixelRatio || 1;
      const cap =
        performanceModeRef.current === 'background'
    ...
    let renderPixelRatio = getRenderPixelRatio(width);
    ...
    const updateGraphLabels = (
      ...
      lockedGraphLabels.forEach((label, index) => {
        ...
        const isVisible =
          ...
          labelProjection.y < 1.18;
        const labelSafePadding = width < 720 ? 18 : 28;
        const labelBounds = label.getBoundingClientRect();
        const labelWidth = labelBounds.width || label.offsetWidth || 0;
        const labelHeight = labelBounds.height || label.offsetHeight || 0;
    ...
    const resize = () => {
      ...
      const nextRenderPixelRatio = getRenderPixelRatio(nextWidth);
    ...
    const animate = () => {
      ...
      if (frameNow - lastRenderTime < minFrameInterval) {
        animationId = requestAnimationFrame(animate);
        return;
      }

      lastRenderTime = frameNow;
      const nextRenderPixelRatio = getRenderPixelRatio();
      ...
      const graphSpinContainment = smoothstep(0.04, 0.28, graphProgress);
      const containedGraphYaw =
        Math.sin(elapsed * (activeIsAgentOrbitFocus ? 0.42 : 0.36)) *
        (activeIsAgentOrbitFocus ? GRAPH_AGENT_YAW_LIMIT : GRAPH_ROUTE_YAW_LIMIT);
      const innerDiamondSpin = elapsed * INNER_DIAMOND_SPIN_SPEED * (1 - graphSpinContainment) + containedGraphYaw * graphSpinContainment;
      ...
      const visibleSceneSpinTarget = graphTargetProgress > 0 ? containedGraphYaw * GRAPH_LAYER_PARALLAX_YAW : sceneSpin;
      visibleSceneSpin = lerpAngle(visibleSceneSpin, visibleSceneSpinTarget, graphTargetProgress > 0 ? 0.18 : 0.08);
      const graphLayerSpinTarget =
        graphTargetProgress > 0
          ? Math.sin(elapsed * (activeIsAgentOrbitFocus ? 0.36 : 0.3)) *
            (activeIsAgentOrbitFocus ? GRAPH_AGENT_YAW_LIMIT : GRAPH_ROUTE_YAW_LIMIT) *
            GRAPH_LAYER_PARALLAX_YAW
          : visibleSceneSpin;
      if (graphTargetProgress > 0 && localGraphReveal < 0.01) {
        graphLayerSpin = graphLayerSpinTarget;
      } else {
        graphLayerSpin = lerpAngle(graphLayerSpin, graphLayerSpinTarget, graphTargetProgress > 0 ? 0.08 : 0.14);
      }
      points.rotation.y = visibleSceneSpin;
      points.rotation.x = Math.sin(elapsed * 0.07) * 0.012 * (1 - graphBlend * 0.62);
