...
import { fetchRecommendationSnapshot, saveRecommendationLineup } from '../../lib/recommendationSnapshotClient';
import type { AgentCatalogItem, RecommendationSnapshot, RecommendedAgent } from '../../types';
...
function getSnapshotSavedLineupAgents(snapshot: RecommendationSnapshot | null): Array<RecommendedAgent | null> {
...
function createSavedLineupAgent(agent: EnrichedDrawAgent, slotIndex: number): RecommendedAgent {
  ...
  return {
    ...
    rank: slotIndex + 1,
    reason,
    source: String(agent.source || 'saved_lineup'),
    stage,
...
export function AgentCombinationEntryPage({ recommendationId }: { recommendationId: string }) {
  ...
  const savedLineupAgents = useMemo(
    () => getSnapshotSavedLineupAgents(snapshot).map((agent) => (agent ? enrichDrawAgent(agent) : null)),
    [catalogAgents, snapshot],
  );
  ...
  const canSaveLineup = filledLineupAgents.length > 0 && !loading && !error;
  const isLineupSaved = saveStatus === 'saved' && currentLineupSignature === (lastSavedLineupSignatureRef.current || savedLineupSignature);
  const saveButtonLabel = saveStatus === 'saving' ? '保存中' : isLineupSaved ? '已保存' : '保存阵容';
  const saveFeedbackText = saveStatus === 'error' ? saveError : isLineupSaved ? '当前组合已保存到推荐快照' : '';

  useEffect(() => {
    if (hasSavedLineup && seededLineupSignatureRef.current !== `saved:${savedLineupSignature}`) {
  ...
  const saveCurrentLineup = useCallback(async () => {
    ...
    try {
      ...
      const updatedSnapshot = await saveRecommendationLineup(
        recommendationId,
        savedLineup,
        lineupScore as unknown as Record<string, unknown>,
      );
      ...
      setSaveError(nextError instanceof Error ? nextError.message : '阵容保存失败');
      setSaveStatus('error');
    }
  }, [canSaveLineup, currentLineupSignature, lineupAgents, lineupScore, recommendationId, saveStatus]);
  ...
  const addAgentToLineup = useCallback(
    (agentKey: string) => {
      if (!lineupCandidateByKey.has(agentKey)) {
        return;
      }
  ...
  const commitLineupDrop = useCallback(
    (payload: LineupDragPayload, targetSlotIndex: number) => {
      if (!payload.agentKey || !lineupCandidateByKey.has(payload.agentKey)) {
        return;
      }
  ...
  const handleSlotDrop = useCallback(
    ...
    [clearLineupDragState, commitLineupDrop, dragPayload, lineupCandidateByKey],
  );
  ...
  const handleLineupPointerDragStart = useCallback(
    (event: ReactPointerEvent<HTMLElement> | ReactMouseEvent<HTMLElement>, payload: LineupDragPayload) => {
      if (event.button !== 0 || (event.target as HTMLElement).closest('a, input, select, label, .agent-combination-slot-actions')) {
        return;
      }
    ...
    [clearLineupDragState, commitLineupDrop],
  );

  return (
    <main className="agent-combination-entry-page">
      <div className="agent-combination-entry-frame">
        <div className="agent-combination-save-bar" aria-live="polite">
          <button
            className={`agent-combination-save-button ${isLineupSaved ? 'is-saved' : ''}`}
            disabled={!canSaveLineup || saveStatus === 'saving' || isLineupSaved}
            onClick={saveCurrentLineup}
            type="button"
          >
            {isLineupSaved ? <CheckCircle2 size={16} /> : <Save size={16} />}
            <span>{saveButtonLabel}</span>
          </button>
          {saveFeedbackText ? <span className={`agent-combination-save-feedback is-${saveStatus}`}>{saveFeedbackText}</span> : null}
        </div>
