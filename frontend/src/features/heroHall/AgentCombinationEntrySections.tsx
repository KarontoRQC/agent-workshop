  ...
  return (
    <section className="agent-combination-entry-hero" aria-label="智能体英雄殿堂">
      <div className="agent-combination-hero-copy">
        <div className="agent-combination-hero-kicker-row">
...
export function AgentLineupBuilder({
  ...
  return (
    <section className="agent-combination-lineup-builder" aria-label="组合智能体" data-read-only={!isEditable}>
      <div className="agent-combination-lineup-header">
        ...
        <div className="agent-combination-lineup-actions">
          ...
          <button
            className={`agent-combination-lineup-save ${isLineupSaved ? 'is-saved' : ''}`}
            disabled={!isEditable || !canSaveLineup || saveStatus === 'saving' || isLineupSaved}
            onClick={onSaveLineup}
            type="button"
          >
            {isLineupSaved ? <CheckCircle2 size={15} /> : <Save size={15} />}
            <span>{saveButtonLabel}</span>
          </button>
          <button className="agent-combination-lineup-reset" disabled={!isEditable || !hasLineupAgent} onClick={onClearLineup} type="button">
            <RefreshCcw size={15} />
            <span>重置阵容</span>
          </button>
          {saveFeedbackText ? <span className={`agent-combination-lineup-save-feedback is-${saveStatus}`}>{saveFeedbackText}</span> : null}
        </div>
      </div>
      ...
      {agent ? (
        <>
          ...
          <div className="agent-combination-slot-actions">
            ...
            <button aria-label={`移除${agent.name}`} disabled={!isEditable} onClick={() => onRemoveSlot(index)} type="button">
              <X size={14} />
            </button>
          </div>
        </>
